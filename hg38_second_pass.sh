...
#list all alignment files in directory and print their names
...
#sort .bam by name
samtools sort -n ${FILENAME}_HISAT2_virome_alignment_first_pass.bam > ${FILENAME}_HISAT2_virome_alignment_first_pass_sorted_names.bam

#extracting mapped reads from pseudovirome alignment
samtools view -h -F ${FILENAME}_HISAT2_virome_alignment_first_pass_sorted_names.bam > ${FILENAME}HISAT2_virome_alignment_first_pass_sorted_names_mappedReads.sam

#convert to bam
samtools view -Su ${FILENAME}HISAT2_virome_alignment_first_pass_sorted_names_mappedReads.sam > ${FILENAME}HISAT2_virome_alignment_first_pass_sorted_names_mappedReads.bam

#bam to fastq
bedtools bamtofastq -i ${FILENAME}HISAT2_virome_alignment_first_pass_sorted_names_mappedReads.bam -fq ${FILENAME}_R1.fq -fq2 ${FILENAME}_R2.fq
